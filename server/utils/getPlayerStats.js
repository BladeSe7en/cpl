...
function getPlayerStats(names) {
    console.log('names[0].names: ', names[0].names)
    return new Promise((resolve, reject) => {
        const { Player } = app.models;
            const playerData = names[0].names.map(name => {
                ...
                .then(() => {
                   if(!name) {
                       let newPlayer = {
                        ...
                        Player.findOrCreate(newPlayer)
                        .then(() => resolve(newPlayer))
                        .catch(err => reject(err))
                    }
                    Player.findOrCreate(name)
                        .then(() => resolve(name))
                        .catch(err => reject(err))
                })
